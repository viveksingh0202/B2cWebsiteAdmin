...
{
    public class ClsCashManagement:GetConnectionString
    {
        public DataTable GetDataCashManagement()
        {
            try
            {
                string strcon = getconnection();
                SqlConnection con = new SqlConnection(strcon);
                SqlCommand cmd = new SqlCommand("Sp_TblCashManagementGet", con);
        ...
        public int DataInsertCashManagement(string image1, string image2, string Heading1, string Heading2, string content1, string content2,  string buttonLink, string create_updateby)
        {
            try
            {
                ...
                string strcon = getconnection();
                SqlConnection con = new SqlConnection(strcon);
                SqlCommand cmd = new SqlCommand("Sp_TblCashManagement", con);
                cmd.CommandType = CommandType.StoredProcedure;
