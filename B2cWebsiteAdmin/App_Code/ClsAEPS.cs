...
{
    public class ClsAEPS:GetConnectionString
    {
        public DataTable GetDataAEPS()
        {
            try
            {
                string strcon = getconnection();
                SqlConnection con = new SqlConnection(strcon);
                SqlCommand cmd = new SqlCommand("Sp_TblAEPSGet", con);
        ...
        public int DataInsertAEPS(string image1, string image2, string image3, string Heading1, string Heading2, string Heading3, string content1, string content2, string content3, string paragraph1, string paragraph2, string paragraph3, string buttonLink, string create_updateby)
        {
            try
            {
                ...
                string strcon = getconnection();
                SqlConnection con = new SqlConnection(strcon);
                SqlCommand cmd = new SqlCommand("Sp_TblAEPS", con);
                cmd.CommandType = CommandType.StoredProcedure;
