...
{
    public class ClsMicroATM:GetConnectionString
    {
        public DataTable GetDataMicroAtm()
        {
            try
            {
                string strcon = getconnection();
                SqlConnection con = new SqlConnection(strcon);
                SqlCommand cmd = new SqlCommand("Sp_TblMicroATMGet", con);
        ...
        public int DataInsertMicroAtm(string image1, string image2,  string Heading1, string Heading2,  string content1, string content2, string buttonLink, string create_updateby)
        {
            try
            {
                ...
                string strcon = getconnection();
                SqlConnection con = new SqlConnection(strcon);
                SqlCommand cmd = new SqlCommand("Sp_TblMicroATM", con);
                cmd.CommandType = CommandType.StoredProcedure;
